    ...
    return (
        <div className='max-w-lg mx-auto px-2'>
            <div className='flex flex-col gap-3 items-center mb-4'>
                ...
                <p className='text-yellow-400 text-center text-sm'>This phrase is the ONLY way to recover your wallet. DO NOT share it with anyone.</p>
            </div>
            ...
            <div className={`grid grid-cols-2 md:grid-cols-3 gap-2 justify-center w-full items-center mx-auto my-6 transition-all duration-300 ${!isSecretShown ? "blur-sm pointer-events-none select-none" : ""}`}>
                {mnemonicWords.map((word, idx) => (
                    <p
                    key={idx}
                    className="md:text-lg bg-zinc-700 hover:bg-zinc-800/80 transition-all duration-300 rounded-lg px-3 p-2 text-start"
                    >
                        <span className='text-base text-gray-400 mr-0.5'>{idx + 1}.</span> {word}
                    </p>
                ))}
            </div>
